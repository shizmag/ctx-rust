...
pub fn ensure_index(
    ...
    if !db_path.exists() {
        let _ = rebuild_index_db_with_registry(&workspace_root, options.clone(), global_registry())?;
    } else if let Ok(state) = get_index_state_with_registry(&workspace_root, &options, global_registry()) {
