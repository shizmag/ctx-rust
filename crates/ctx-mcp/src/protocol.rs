...
pub fn depth_or_auto(args: &serde_json::Value) -> Result<ctx_codegraph::DepthLimit, String> {
    match args.get("depth") {
        None => Ok(ctx_codegraph::DepthLimit::Auto),
        Some(v) if v.as_str() == Some("auto") => Ok(ctx_codegraph::DepthLimit::Auto),
        Some(v) => v
            .as_u64()
            .map(|d| ctx_codegraph::DepthLimit::Fixed(d as usize))
            .or_else(|| v.as_i64().map(|d| ctx_codegraph::DepthLimit::Fixed(d as usize)))
            .ok_or_else(|| "depth must be a non-negative integer or \"auto\"".to_string()),
    }
}

pub fn parse_graph_context_mode(mode_str: &str) -> ctx_codegraph::model::GraphContextMode {
    match mode_str {
        "callers" => ctx_codegraph::model::GraphContextMode::Callers,
        "callees" => ctx_codegraph::model::GraphContextMode::Callees,
        "dependencies" => ctx_codegraph::model::GraphContextMode::Dependencies,
        "dependents" => ctx_codegraph::model::GraphContextMode::Dependents,
        "forward-slice" | "forward_slice" => ctx_codegraph::model::GraphContextMode::ForwardSlice,
        "reverse-slice" | "reverse_slice" => ctx_codegraph::model::GraphContextMode::ReverseSlice,
        "forward" => ctx_codegraph::model::GraphContextMode::Forward,
        "reverse" => ctx_codegraph::model::GraphContextMode::Reverse,
        "impact" => ctx_codegraph::model::GraphContextMode::Impact,
        _ => ctx_codegraph::model::GraphContextMode::Neighborhood,
    }
}

pub fn parse_edge_kinds(values: &[String]) -> Vec<ctx_codegraph::model::EdgeKind> {
    values
        .iter()
        .filter_map(|k| {
            ctx_codegraph::model::EdgeKind::from_str(k).or_else(|| {
                ...
                ctx_codegraph::model::EdgeKind::from_str(&capitalized)
            })
        })
        .collect()
}
...
mod tests {
    ...
    fn depth_or_auto_parses_variants() {
        ...
        assert!(matches!(
            ...
            ctx_codegraph::DepthLimit::Fixed(3)
        ));
