...
fn rebuild_affected_edges_in_tx_with_registry(
    ...
    for cs in &occurrences {
        ...
        let use_lsp = options.should_use_full_lsp()
            || ctx_codegraph_lang::pipeline::should_use_light_lsp(options);
        if use_lsp
...
pub fn run_full_rebuild_with_registry(
    ...
    write_meta(&tx, "schema_version", "7")?;
    ...
    let target_tier = options.target_tier();
    write_meta(&tx, "extraction_tier", target_tier.as_str())?;
    write_meta(&tx, "pipeline_version", "1.0.0")?;

    if target_tier >= ctx_codegraph_lang::model::ExtractionTier::Balanced {
        super::metrics::compute_and_save_graph_metrics_in_tx(&tx)?;
    }

    if options.should_use_full_lsp() {
        write_meta(&tx, "lsp_enrichment", "complete")?;
    } else if ctx_codegraph_lang::pipeline::should_use_light_lsp(&options) {
        write_meta(&tx, "lsp_enrichment", "light")?;
    } else {
