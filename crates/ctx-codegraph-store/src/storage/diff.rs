...
pub fn compute_index_diff_with_registry(
        ...
        .filter_entry(|e| {
            ...
            if path.is_dir()
                && let Some(name) = path.file_name().and_then(|n| n.to_str())
                    && ctx_codegraph_lang::discovery::should_skip_dir(name) {
                        return false;
                    }
            true
        });
    for entry in walker.filter_map(|e| e.ok()) {
        let path = entry.path();
        if path.is_file()
            && ctx_codegraph_lang::index::should_index_path_with_registry(path, registry) {
    ...
    for path in &disk_files {
        let disk_mtime = ctx_codegraph_lang::index::get_mtime_ms(path).unwrap_or(0);
        let disk_size = ctx_codegraph_lang::index::get_size_bytes(path).unwrap_or(0) as u64;
        ...
        )) = db_files.get(path)
        {
            ...
            let is_modified = match options.change_detection {
                ...
                FileChangeDetection::ContentHash => {
                    let computed = ctx_codegraph_lang::index::compute_file_hash(path);
