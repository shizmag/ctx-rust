...
impl ResolverBackend for PythonResolver {
    fn resolver_id(&self) -> ResolverId {
        ResolverId::new("pyright-lsp")
    }
    ...
    fn resolve(&self, input: ResolveInput<'_>) -> Result<ResolveOutput, CodeGraphError> {
        ...
        if needs_new_client {
            *client_lock = None;
            match GenericLspClient::new(input.workspace_root, "pyright-langserver", &["--stdio"]) {
                Ok(mut c) => {
                ...
                Err(err) => {
                    eprintln!(
                        "Warning: Failed to start pyright LSP: {}. Falling back to name-only resolution.",
                        err
                    );
                }
            }
