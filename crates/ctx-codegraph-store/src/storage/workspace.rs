...
pub fn find_workspace_root(start_dir: &Path, registry: &BackendRegistry) -> PathBuf {
        ...
        if !matches {
            for backend in registry.all() {
                for marker in backend.workspace_markers() {
                    match marker {
                        ctx_codegraph_lang::backend::WorkspaceMarker::File(name) => {
                            if current.join(name).exists() {
                                matches = true;
                                break;
                            }
                        }
                        ctx_codegraph_lang::backend::WorkspaceMarker::Directory(name) => {
                            if current.join(name).exists() {
                                matches = true;
                                break;
                            }
...
pub fn open_db(
    ...
    open_codegraph_db(root, registry)
}
...
pub fn read_metadata(root: &Path, registry: &BackendRegistry, key: &str) -> Option<String> {
    let workspace_root = find_workspace_root(root, registry);
    let db_path = workspace_root.join(".ctx-codegraph/codegraph.sqlite");
    if !db_path.exists() {
        return None;
