...
pub fn init_schema(
    ...
    let metas: Vec<_> = registry
        .all()
        .iter()
        .map(|b| b.metadata(&ctx_codegraph_lang::index::BuildIndexOptions::default()))
        .collect();
...
mod tests {
    ...
    fn test_initializes_schema() {
        ...
        let conn = open_db(dir.path(), &registry).unwrap();
    ...
    fn test_read_write_metadata_helpers() {
        ...
        assert!(read_metadata(dir.path(), &registry, "k").is_none());
        assert!(write_metadata(dir.path(), &registry, "k", "v").is_err());
        assert!(!db_path.exists());
        ...
        write_metadata(dir.path(), &registry, "mcp_last_stats", "{\"calls\":5}").unwrap();
        assert_eq!(
            read_metadata(dir.path(), &registry, "mcp_last_stats").as_deref(),
            Some("{\"calls\":5}")
        );
        write_metadata(dir.path(), &registry, "schema_version", "test").unwrap();
        assert_eq!(
            read_metadata(dir.path(), &registry, "schema_version").as_deref(),
            Some("test")
        );
        assert!(read_metadata(dir.path(), &registry, "missing").is_none());
    }
}
